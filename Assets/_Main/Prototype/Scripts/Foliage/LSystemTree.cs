...
public class LSystemTree : MonoBehaviour
{
    ...
    public float leafSize = 2;
    public bool projectLeafNormals = true;
    public Vector3 leafNormalsOrigin;
    ...
    void GenerateFoliage(Mesh mesh = null)
    {
        ...
        for (int i = 0; i < sentence.Length; i++)
        {
            ...
            else if (currentChar == 'L') // Add a leaf
            {
                if (generateLeaves)
                {
                    Leaf newLeaf = new Leaf(workingBranchState.transform, leafSize);
                    leaves.Add(newLeaf);
                }
            }
        ...
        if (mesh != null)
        {
            ...
            {
                leafMeshInstances.Add(LeafMeshBuilder.CreateLeaf(leaf));
            }
            ...
            if (projectLeafNormals)
            {
                LeafMeshBuilder.ProjectNormals(leavesMesh, leafNormalsOrigin);
            }
    ...
    void OnDrawGizmos()
    {
        ...
        if (drawLeafNodes)
        {
            ...
            foreach (Leaf leaf in leaves)
            {
                Gizmos.DrawSphere(leaf.transform.GetPosition(), 0.1f);
            }

            if (projectLeafNormals)
            {
                Gizmos.color = Color.red;
                Gizmos.DrawSphere(leafNormalsOrigin, 0.3f);
            }
        }
    }
}
